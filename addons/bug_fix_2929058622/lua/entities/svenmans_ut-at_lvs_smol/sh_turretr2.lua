function ENT:SetPoseParameterTurretR2( weapon )
	
	--if not weapon:GetAI() then return end

	if not IsValid(self:GetTurretR2Seat()) or not IsValid(self:GetTurretR2Driver()) then return end

	local Pod = self:GetTurretR2Seat()
	local Driver = self:GetTurretR2Driver()
	...
	self:SetPoseParameter("turretR2_hight", TargetAng.p)
	self:ManipulateBoneAngles(self:LookupBone("turretR2"), Angle(y,0,0) )

end


function ENT:InitTurretR2()
	...
	weapon.Attack = function( ent )
		...
		--base:PlayAnimation( "turretR2_fire" )

		local ID = base:LookupAttachment( "turretR2" )
		local Muzzle = base:GetAttachment( ID )
		
		if not Muzzle then return end
		
		if not IsValid(base:GetTurretR2Driver()) or not IsValid(base:GetTurretR2Seat()) then return end

		local Driver = base:GetTurretR2Driver()
		local Pod = base:GetTurretR2Seat()
	...
	weapon.OnThink = function( ent, active )
		...
		base:SetPoseParameterTurretR2( ent )
	end
	...
	weapon.CalcView = function( ent, ply, pos, angles, fov, pod )
		...
		if pod:GetThirdPersonMode() then
			...
			local radius = 100
			local viewbone = base:GetAttachment(base:LookupAttachment( "turretR2"))
	...
	weapon.HudPaint = function( ent, X, Y, ply )
		...
		local ID = self:LookupAttachment( "turretR2" )
