...
-- Misc --
ENT.PrintName = "Rancor-dragon"
ENT.Category = "[Valkyrie] Creature NPCs"
ENT.Models = {"models/swtor/npcs/rancordragon_a01.mdl"}
ENT.Skins = {0}
ENT.ModelScale = 1
...
-- Animations --
ENT.WalkAnimation = ACT_IDLE
ENT.WalkAnimRate = 1
ENT.RunAnimation = ACT_IDLE
ENT.RunAnimRate = 1
...
if SERVER then
  ...
  if math.random(1,2) == 1 then
	...
	elseif ( enemy:GetClass() == "npc_turret_floor" ) then
		enemy:SetMoveType(MOVETYPE_VPHYSICS)
		enemy:PhysicsInit(SOLID_VPHYSICS)
	else
  ...
  function ENT:OnDeath(dmg, hitgroup) 
	...
	self:PlaySequenceAndMove("die", 1, self.FaceEnemy)
  end
