...
function ENT:Initialize()
    self:SetModel("models/funky/starwars/gunship.mdl")
    ...
    self:SetPos(self:GetPos() + Vector(-2000, 0, 1700)) -- Inicializa a nave no céu
    self:SetAngles(Angle(0, -90, 0)) -- Define a orientação inicial da nave para apontar para o norte (Y=0)
    self.TargetPos = self:GetPos() + Vector(2000, 0, -1400) -- Posição de descida para ficar flutuando a 50 unidades acima do solo
    self.Deployed = false
    self.Arrived = false
    self.Landing = false
    self.Floating = false
    self.SoundLoop = CreateSound(self, "lvs/vehicles/vulturedroid/loop.wav") -- Som contínuo
    self.SoundLoop:Play()

    self.FloatAmplitude = 50 -- Amplitude da flutuação
...
function ENT:Deploy()
    -- Toca o som de pouso
    self:EmitSound("lvs/vehicles/vulturedroid/flyby.wav")
...
function ENT:FlyRight()
    self.TargetPos = self:GetPos() + -self:GetRight() * 3000 -- Define um alvo de voo para frente
...
function ENT:SpawnNPCs()
    local positions = {
        Vector(300, 100, -300), Vector(300, 0, -300), Vector(300, -100, -300), Vector(300, -200, -300),
        Vector(300, -300, -300), Vector(300, 200, -300),Vector(200, -300, -300), Vector(200, -200, -300), Vector(200, -100, -300),
        Vector(200, 0, -300), Vector(200, 100, -300), Vector(200, 200, -300)
    }
    ...
    for i, offset in ipairs(positions) do
        ...
        local spawnAng = Angle(0, 0, 0) -- Define o ângulo para que os NPCs fiquem em pé
