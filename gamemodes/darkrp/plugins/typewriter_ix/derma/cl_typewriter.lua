...
function PANEL:Init()
    ...
    self.Paint = function(self, w, h)
        ...
        draw.DrawText("Impressora", "TypewriterTitleFont", w * 0.5, 8, color_white, TEXT_ALIGN_CENTER)
    end
    ...
    self.OpenText:SetText("Use esta máquina de escrever para transformar um Documento Google em um item do jogo em seu inventário.")
	...
	self.Title:Dock(TOP)
	self.Title:SetPlaceholderText("Título do documento")

	self.Body = self:Add("DTextEntry")
	self.Body:SetZPos(3)
	self.Body:DockMargin(0, 15, 0, 0)
	self.Body:Dock(TOP)
	self.Body:SetPlaceholderText("Corpo do documento (Google Doc Link)")
    ...
    self.OpenText:SetText("Quantas cópias você gostaria? (Max " .. ix.config.Get("MaxQuantityAmount") .. ")")
	...
	self.CreateButton:SetText("Criar documento")
