function ENT:SetPoseParameterTurretL1( weapon )
	
	--if not weapon:GetAI() then return end

	if not IsValid(self:GetTurretL1Seat()) or not IsValid(self:GetTurretL1Driver()) then return end

	local Pod = self:GetTurretL1Seat()
	local Driver = self:GetTurretL1Driver()
	...
	self:SetPoseParameter("turretL1_hight", TargetAng.p)
	self:ManipulateBoneAngles(self:LookupBone("turretL1"), Angle(y,0,0) )

end


function ENT:InitTurretL1()
	...
	weapon.Attack = function( ent )
		...
		local ID = base:LookupAttachment( "turretL1" )
		local Muzzle = base:GetAttachment( ID )
		
		if not Muzzle then return end
		
		if not IsValid(base:GetTurretL1Driver()) or not IsValid(base:GetTurretL1Seat()) then return end

		local Driver = base:GetTurretL1Driver()
		local Pod = base:GetTurretL1Seat()
	...
	weapon.OnThink = function( ent, active )
		...
		base:SetPoseParameterTurretL1( ent )
	end
	...
	weapon.CalcView = function( ent, ply, pos, angles, fov, pod )
		...
		if pod:GetThirdPersonMode() then
			...
			local radius = 100
			local viewbone = base:GetAttachment(base:LookupAttachment( "turretL1"))
	...
	weapon.HudPaint = function( ent, X, Y, ply )
		...
		local ID = self:LookupAttachment( "turretL1" )
	...
	weapon.Attack = function( ent )
		...
		--base:PlayAnimation( "turretL1_fire" )
