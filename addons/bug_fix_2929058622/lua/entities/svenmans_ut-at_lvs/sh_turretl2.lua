function ENT:SetPoseParameterTurretL2( weapon )
	
	--if not weapon:GetAI() then return end

	if not IsValid(self:GetTurretL2Seat()) or not IsValid(self:GetTurretL2Driver()) then return end

	local Pod = self:GetTurretL2Seat()
	local Driver = self:GetTurretL2Driver()
	...
	self:SetPoseParameter("turretL2_hight", TargetAng.p)
	self:ManipulateBoneAngles(self:LookupBone("turretL2"), Angle(y,0,0) )

end


function ENT:InitTurretL2()
	...
	weapon.Attack = function( ent )
		...
		local ID = base:LookupAttachment( "turretL2" )
		local Muzzle = base:GetAttachment( ID )
		
		if not Muzzle then return end
		
		if not IsValid(base:GetTurretL2Driver()) or not IsValid(base:GetTurretL2Seat()) then return end

		local Driver = base:GetTurretL2Driver()
		local Pod = base:GetTurretL2Seat()
	...
	weapon.OnThink = function( ent, active )
		...
		base:SetPoseParameterTurretL2( ent )
	end
	...
	weapon.CalcView = function( ent, ply, pos, angles, fov, pod )
		...
		if pod:GetThirdPersonMode() then
			...
			local radius = 100
			local viewbone = base:GetAttachment(base:LookupAttachment( "turretL2"))
	...
	weapon.HudPaint = function( ent, X, Y, ply )
		...
		local ID = self:LookupAttachment( "turretL2" )
