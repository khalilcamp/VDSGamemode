...
if (SERVER) then
	...
	function PLUGIN:PlayerTick(ply, mv)
		...
		if ply:GetNWBool("ConstructablePropPlacing", false) then
			if !ply:Crouching() and IsValid(ply) and ply:Alive() and IsValid(wep) and wep:GetClass() == "mvp_hands" and pos:Distance(tr.HitPos) <= 400 then
	...
	function PLUGIN:KeyPress(ply, key)
		if key == IN_USE and IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "mvp_hands" and ply:GetEyeTrace().HitPos:Distance(ply:GetPos()) <= 400 then
			if ply:GetNWBool("ConstructablePropPlacing") then
				for k, v in pairs(player.GetAll()) do
					if ply.propConstructHolo:GetPos():Distance(v:GetPos()) <= ply.propConstructHolo:BoundingRadius()*1 then
		...
		if key == IN_RELOAD and IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "mvp_hands" and ply:GetEyeTrace().HitPos:Distance(ply:GetPos()) <= 400 then
			if ply:GetNWBool("ConstructablePropPlacing") then
				ply:SetNWString( "ConstructablePropModel", "nul")
				ply.propConstructHolo:Remove()
				ply:SetNWBool("ConstructablePropPlacing", false)
                ...
                ply:SetNWBool("ConstructablePropPlacing", false)
                ply:SetNWBool("ConstructablePropReadytoPlace", false)
                ply:SetNWString( "ConstructablePropModel", "nul")
                ply:SetNWInt( "ConstructablePropID", 0 )
                ply:SetNWInt( "ConstructablePropRotation", 0 )
                ply:SetNWInt( "ConstructablePropRotationX", 0 )
			end
		end
		if key == IN_ATTACK and IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "mvp_hands" then
			if ply:GetNWBool("ConstructablePropPlacing") then
		...
		if key == IN_ATTACK2 and IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "mvp_hands" then
			if ply:GetNWBool("ConstructablePropPlacing") then
...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if lclient:GetNWBool("ConstructablePropPlacing", false) then
			...
			if lclient:GetActiveWeapon():GetClass() != "mvp_hands" then
